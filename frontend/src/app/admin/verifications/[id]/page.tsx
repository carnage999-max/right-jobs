  ...
  return (
    <div className="max-w-6xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex items-center justify-between">
        <Button variant="ghost" className="ios-button h-10 px-4 font-bold text-slate-500 hover:bg-slate-100 rounded-xl" onClick={() => router.back()}>
              ...
              <CardContent className="p-8 space-y-6">
                 <div className="flex items-center gap-5">
                    ...
                    <div className="overflow-hidden">
                       <h4 className="font-black text-xl text-slate-900 tracking-tight leading-tight">{verification.user.name || "Unnamed"}</h4>
                       <p className="text-xs font-bold text-slate-400 flex items-center gap-1.5 mt-1">
                          <Mail className="h-3 w-3" /> {verification.user.email}
                       </p>
                    </div>
                 </div>
        ...
        <div className="space-y-8">
           <Card className="ios-card shadow-2xl shadow-slate-200/60 overflow-hidden">
              <CardHeader className="flex flex-row items-center justify-between border-b px-8 py-6">
                 <div>
                    <CardTitle className="text-2xl font-black tracking-tight text-slate-900">Submission Assets</CardTitle>
                 ...
                 <div className="flex gap-3">
                    <Button variant="outline" className="ios-button h-10 px-4 font-bold border-2" size="sm">
              ...
              <CardContent className="p-0">
                 <div className="divide-y divide-slate-100">
                    {documents.map((doc, i) => (
                       <div key={i} className="p-10 hover:bg-slate-50/30 transition-all group">
                          <div className="flex items-center justify-between mb-8">
                             <div className="flex items-center gap-5">
                                ...
                                <div>
                                   <h5 className="font-black text-lg text-slate-900 leading-tight">{doc.type}</h5>
                                   <div className="flex gap-2 mt-1">
                                      <Badge variant="secondary" className="text-[9px] font-black tracking-tighter rounded-sm px-1.5 py-0">{doc.label}</Badge>
                                      <span className="text-xs font-bold text-slate-400 uppercase tracking-widest">Secure Cloud Storage</span>
                                   </div>
                                </div>
                             </div>
                             <Button variant="outline" className="ios-button h-10 px-5 font-bold border-2 group-hover:bg-primary group-hover:text-white group-hover:border-primary transition-all shadow-md active:scale-95" asChild>
                          ...
                          <div className="relative group/img overflow-hidden rounded-[2rem] border-4 border-white shadow-2xl ring-1 ring-slate-100">
                             <img 
                                src={doc.url} 
                                alt={doc.type} 
                                className="w-full h-auto object-cover max-h-[600px] transition-transform duration-700 group-hover/img:scale-105"
                             />
                             <div className="absolute inset-0 bg-slate-900/40 opacity-0 group-hover/img:opacity-100 transition-opacity flex items-center justify-center">
                                <Button variant="secondary" className="ios-button font-black rounded-full px-8 h-12 shadow-2xl" asChild>
           ...
           <div className="flex gap-6 pb-20">
              <Button 
                variant="outline" 
                className="flex-1 ios-button h-16 border-2 border-red-200 text-red-600 hover:bg-red-50 hover:text-red-700 font-black text-lg shadow-xl shadow-red-100/50 rounded-2xl"
                onClick={() => handleAction("REJECTED")}
                disabled={isProcessing}
              >
                 {isProcessing ? <Loader2 className="animate-spin" /> : <XCircle className="mr-3 h-6 w-6" />} Reject Dossier
              </Button>
              <Button 
                className="flex-[2] ios-button h-16 font-black text-lg shadow-2xl shadow-primary/30 rounded-2xl"
                onClick={() => handleAction("VERIFIED")}
                disabled={isProcessing}
              >
                 {isProcessing ? <Loader2 className="animate-spin" /> : <ShieldCheck className="mr-3 h-6 w-6" />} Authorize & Verify Identity
              </Button>
           </div>
        </div>
      </div>
