...
// Helper to get cities - defaults to major US and World cities if country not found or not selected
export const getCities = (countryName?: string) => {
  if (countryName && citiesByCountry[countryName]) {
    return citiesByCountry[countryName];
  }
  // If no country selected or not in list, return a mix of major world cities
  return [
